...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = runner.run("Explain this code", Some("You are a code reviewer"), "sonnet", None, None, 600)?;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = runner.run_async("Summarize", Some("Be concise"), "opus", None, None, 600).await?;
    println!("{}", result.output);
    Ok(())
}
```
...
### `runner.run(prompt, system_prompt, model, tools, cwd, timeout)`
